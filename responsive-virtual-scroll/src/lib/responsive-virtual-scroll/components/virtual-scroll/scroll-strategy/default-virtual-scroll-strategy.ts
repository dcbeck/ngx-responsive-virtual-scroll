...
@Injectable()
export class DefaultVirtualScrollStrategy<T>
  implements VirtualScrollStrategy<T>
{
  ...
  public renderViewForItem(
    ...
    if (view) {
      ...
      if (deferViewCreation) {
        ...
        const placeholderViewRef =
          scrollState.viewContainerRef.createEmbeddedView(
    ...
    if (view.placeholder) {
      result$ = of(view).pipe(
        ...
        map((placeholderView) => {
          ...
          const placholderIndex = scrollState.viewContainerRef.indexOf(
            placeholderView.viewRef
          );
          scrollState.viewContainerRef.remove(placholderIndex);

          // Create the real view where the placeholder used to be
          const asyncView = {
            viewRef: this.createViewRefForItem(
              scrollState,
              item,
              globalIndex,
              placholderIndex
  ...
  public purgeViewCache(scrollState: VirtualScrollState<T>): void {
    if (this.cacheFull(scrollState)) {
      ...
      for (
        let i = startIndex;
        i != endIndex && this.cacheFull(scrollState);
        i += direction
      ) {
        ...
        // If this view isn't about to be rendered, evict it from the cache and destroy it
          ...
          view.itemIndex < minIndex ||
          view.itemIndex >= minIndex + scrollState.renderedItems.length
        ) {
          this.destroyView(scrollState, view);
        }
      }
